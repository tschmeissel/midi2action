...
final public class EventReceiver
	implements	Receiver
{
...
//	final private MidiActionListener midiActionListener;
	...
	public EventReceiver(boolean bPrintTimeStampAsTicks)
	{
		m_bPrintTimeStampAsTicks = bPrintTimeStampAsTicks;
		
//		this.midiActionListener = new MidiActionListener();
	}
	...
	public void send(MidiMessage message, long lTimeStamp)
	{
		...
		else if (message instanceof SysexMessage)
		{
			...
			if ("Sysex message: F0 43 7E 01 00 1C 31 5C F7".equals(strMessage))
			{
				System.exit(0);
			}
		}
		...
		if (isMessageForAction(strMessage))
		{
			// channel 8: program change 52
//			int actionNumber = getActionNumber(strMessage);

//			this.midiActionListener.performAction(actionNumber);
		}
		...
		if (isMessageToDump(strMessage))
		{
			LOGGER.debug(strTimeStamp + strMessage);
			callMessageEventReceiver(strMessage);
		}
	}

//	private int getActionNumber(String strMessage) {
//
//		// [C1 21] channel 2: program change 33
//		String programChangeNumberStr = strMessage.substring(strMessage.lastIndexOf(" "));
//		programChangeNumberStr = programChangeNumberStr.trim();
//		
//		return Integer.valueOf(programChangeNumberStr).intValue();
//	}

	private void callMessageEventReceiver(String strMessage) {
		if (this.messageEventListener != null) {
			this.messageEventListener.sendMessage(strMessage);
		}
	}

	private boolean isMessageToDump(String message) {
		boolean result = true;
		
		if ("[F8] Timing clock".equals(message))
		{
			result = false;
		}
		if ("[FE] Active Sensing".equals(message))
		{
			result = false;
		}
	...
	private boolean isMessageForAction(String message)
	{
		boolean result = false;
		if (message.contains("program change"))
		{
			result = true;
		}
		return result;
